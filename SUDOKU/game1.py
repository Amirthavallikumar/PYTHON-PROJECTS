...
class Sudoku:
    ...
    def draw_cells(self, window, cells, color):
        ...
        for cell in cells:
            pygame.draw.rect(window, color,
                             (gridPos[0] + cell[0] * cellSize, gridPos[1] + cell[1] * cellSize, cellSize, cellSize))

    def drawCell(self, window, pos):
        pygame.draw.rect(window, LIGHTBLUE,
                         (gridPos[0] + pos[0] * cellSize, gridPos[1] + pos[1] * cellSize, cellSize, cellSize))
    ...
    def draw_grid(self, window):
        ...
        for i in range(9):
            ...
            pygame.draw.line(window, BLACK, (gridPos[0] + i * cellSize, gridPos[1]),
                             (gridPos[0] + i * cellSize, gridPos[1] + 450), breadth)
            pygame.draw.line(window, BLACK, (gridPos[0], gridPos[1] + i * cellSize),
                             (gridPos[0] + 450, gridPos[1] + i * cellSize), breadth)

    def OnGrid(self):
        if self.mouse_pos[0] < gridPos[0] or self.mouse_pos[1] < gridPos[1] or self.mouse_pos[0] > gridPos[
            0] + gridSize or self.mouse_pos[1] > gridPos[1] + gridSize:
            return False
                                ...
                                for k in range(3):
                                    for l in range(3):
                                        ...
                                        if self.grid[yidx2][xidx2] == self.grid[yidx][xidx] and [xidx2,
                                                                                                 yidx2] not in self.locked_cells:
    ...
    def Puzzle(self, diff):
        ...
        bsp = BeautifulSoup(source_doc,features="html.parser")
